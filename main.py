...
firebase_admin.initialize_app(cred,{
    'databaseURL':"https://facerecognitionsystem-8dfeb-default-rtdb.asia-southeast1.firebasedatabase.app/",
    'storageBucket' : 'facerecognitionsystem-8dfeb.appspot.com'
})
    ...
    if faceCurFrame:
     ...
     if counter != 0:
        ...
        if counter == 1:
            ...
            blob = bucket.get_blob(f'Images_01/{id}.png')
            array = np.frombuffer(blob.download_as_string(), np.uint8)
            imgEmployee = cv2.imdecode(array, cv2.COLOR_BGRA2BGR)
            # Update data of attendance
            datetimeObject = datetime.strptime(employeeInfo["last_attendace_time"],
                                              "%Y-%m-%d %H:%M:%S")
        ...
        if modeType != 3:
            ...
            if counter <= 10: 
                cv2.putText(imgBackground, str(employeeInfo['total attendance']), (861, 125),
                ...
                cv2.putText(imgBackground, str(employeeInfo['employee status']), (910, 625),
                                cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 100), 1)
                cv2.putText(imgBackground, str(employeeInfo['year']), (1025, 625),
                                cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 100), 1)
                cv2.putText(imgBackground, str(employeeInfo['starting year']), (1125, 625),
                                cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 100), 1)
